...
/**
 * Resize a layout element and handle collisions.
 *
 * Similar to moveElement() but for resize operations. Handles collision
 * detection and cascading movements when an item is resized.
 *
 * **Note**: This function mutates the `l` parameter directly for performance.
 * The item's x, y, w, h, and moved properties will be modified.
 *
 * @param layout - Full layout
 * @param l - Item to resize (will be mutated)
 * @param w - New width
 * @param h - New height
 * @param x - New X position (for west handles: w, nw, sw)
 * @param y - New Y position (for north handles: n, nw, ne)
 * @param isUserAction - True if this is a direct user action
 * @param preventCollision - True to block resize into occupied space
 * @param compactType - Compaction type for collision resolution
 * @param cols - Number of columns in the grid
...
export function resizeElement(
  ...
  // Static items can't be resized unless explicitly resizable
  if (l.static && l.isResizable !== true) {
    return [...layout];
  }
  ...
  const newX = x ?? l.x;
  const newY = y ?? l.y;
  if (l.w === w && l.h === h && l.x === newX && l.y === newY) {
    return [...layout];
  }

  // Update dimensions and position (mutates l directly)
  (l as Mutable<LayoutItem>).w = w;
  (l as Mutable<LayoutItem>).h = h;
  if (typeof x === "number") (l as Mutable<LayoutItem>).x = x;
  if (typeof y === "number") (l as Mutable<LayoutItem>).y = y;
  (l as Mutable<LayoutItem>).moved = true;
  ...
  // Get all collisions with the resized item
  const collisions = getAllCollisions(sorted, l);
  const hasCollisions = collisions.length > 0;
  ...
  // Handle prevent collision mode - revert dimensions
  if (hasCollisions && preventCollision) {
    (l as Mutable<LayoutItem>).w = oldW;
    (l as Mutable<LayoutItem>).h = oldH;
    (l as Mutable<LayoutItem>).x = oldX;
    (l as Mutable<LayoutItem>).y = oldY;
    (l as Mutable<LayoutItem>).moved = false;
    return layout as LayoutItem[];
  ...
  for (let i = 0; i < collisions.length; i++) {
    ...
    // Static items can't be moved - the resize should be constrained
    // but we don't revert here, just skip moving the static item
    if (collision.static) {
      continue;
    }

    // Move the colliding item away from the resized item
    resultLayout = moveElementAwayFromCollision(
      resultLayout,
      l,
      collision,
  ...
  for (let i = 0; i < layout.length; i++) {
    ...
    for (const key of requiredProps) {
      ...
      if (typeof value !== "number" || Number.isNaN(value)) {
        throw new Error(
          `Vue3GridLayout: ${contextName}[${i}].${key} must be a number! ` +
    ...
    if (item.i !== undefined && typeof item.i !== "string") {
      throw new Error(
        `Vue3GridLayout: ${contextName}[${i}].i must be a string! ` +
