/**
 * Core types for vue3-grid-layout
 *
...
/**
 * Data provided during drag operations.
 */
 ...
 * <GridLayout :position-strategy="transformStrategy" />
 *
 * // Use top/left positioning (for environments where transforms cause issues)
 * <GridLayout :position-strategy="absoluteStrategy" />
 *
 * // Use scaled transforms (for scaled containers)
 * <GridLayout :position-strategy="createScaledStrategy(0.5)" />
 * ```
 */
export interface PositionStrategy {
   ...
   */
  calcStyle(pos: Position): CSSProperties;

  /**
   * Calculate position during drag operations, accounting for transforms and scale.
   *
   * This method is optional. When not provided, the drag handler uses built-in
...
export interface ResizeConfig {
   ...
   * Can be a VNode or a function that receives the axis and returns a VNode.
   */
  handleComponent?:
    | VNode
    | ((axis: ResizeHandleAxis, ref: Ref<HTMLElement | null>) => VNode);
}
